...
Both open the same project, [sample-project](sample-project), do the same things and assert the same facts. Nothing to clone by hand: it sits next to the tests. Starter copies it into its own cache, and for the Remote Robot side Gradle copies it into `build/`, so neither run can dirty the checkout.
...
Every command below runs from the `migration-guide/` directory of the checkout.
...
```text
migration-guide/
  sample-project/                     the project both sides open, two Java files
  build/sample-project/               the copy Gradle makes for the Remote Robot side
  starter-driver/
...
| Ready-made UI wrappers | Over a hundred component classes, most of them IDE parts | About twenty fixtures, all plain Swing widgets |
...
|---|---|---|---|
| 1. Build the context | [L24](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L24) | [L24](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L24) | On the Starter side the product, the version, the project and the plugin are values in Kotlin, and `ReusableLocalProjectInfo` copies the project aside on its own, so a test cannot dirty the original. On the Remote Robot side there is no step 1: all of it lives in `build.gradle.kts`, outside anything the test can read or assert, and the project has to be on disk before the test starts. |
| 2. Start the IDE, open the project | [L27](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L27) | [L26](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L26) | Starter installs the plugin, drops `.idea` and every `.iml`, starts the IDE and shuts it down when the test finishes. Remote Robot can only open a project, and it opens whatever `.idea` happens to be on disk. |
| 3. Is the plugin loaded | [L30](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L30) | [L29](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L29) | `isPluginLoaded()` answers with a typed boolean. The Remote Robot equivalent is a JavaScript string over `PluginManagerCore`, written and maintained in the test. |
| 4. Wait for readiness | [L33](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L33) | [L32](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L32) | Driver has one call for it, `waitForIndicators()`: indexing, every progress bar in the status bar, and ten quiet seconds in a row. Remote Robot has `CommonSteps.waitForSmartMode()`, which covers indexing and nothing else, so each scenario below waits for its own component by hand. |
| 5. A project is open | [L37](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L37) | [L37](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L37) | Same idea. `project` is a typed handle versus a JS call returning the project name. |
| 6. The Project View | [L40](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L40) | [L41](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L41) | A fresh IDE opens with the panel showing, and every call on a Driver component resolves it with a retry, so the Starter side neither opens the panel nor waits for the tree to be found. Remote Robot shares one IDE across the suite, so an earlier scenario may have hidden the panel and the test has to put it back. The action id would be shorter, but `CommonSteps.invokeAction()` does not tell the action which project to act on, and it fails silently, so the test clicks the button and checks the state first because a button toggles. For the tree: `waitForNodesLoaded()` versus reading rows until some appear. |
| 7. Expand the source folder | [L45](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L45) | [L60](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L60) | The root node reads back as one label on both sides, `sample-project ~/IdeaProjects`. Driver's `expandPath()` takes `fullMatch = false`, so `sample-project` still matches. Remote Robot's `expand(vararg path)` compares labels exactly and has no `fullMatch`, so expanding goes through a double click. |
| 8. Both files are in the tree | [L50](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L50) | [L73](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L73) | Both sides do the same check here, and neither call changes the tree. But be careful with the name: `isPathExists()` (Remote Robot) looks like `pathExists()` in the Driver API, and they are not the same. `pathExists()` expands the path before it checks, so the test would pass even if step 7 did nothing. This is why the starter side calls `findExpandedPath() != null` instead: it only reads the tree. |
| 9. Open the file by double click | [L57](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L57) | [L78](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L78) | Either library can drop a click on a busy machine, and neither treats that as a failure: Driver logs "Click was unsuccessful" and carries on. So the click has to be repeated rather than waited on. Both sides have the same tool for it, an action plus a check that throws, run again on failure: `withRetries()` on one side and `attempt()` on the other. |
| 10. The right file opened | [L64](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L64) | [L87](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L87) | The Starter side can ask the editor tabs, and step 9 already waited for the tab, so `codeEditorForFile()` only has to hand over the editor to read. The Remote Robot library has no fixture for editor tabs. So the test takes the open editor with `textEditor()` and reads its `fileName`, and that is one more JavaScript call into the IDE. |
...
| Step | Starter and Driver | Remote Robot | What changed                                                                                                                                                                                                                                                                                                                                                                                                                      |
|---|---|---|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1. Open the context menu | [L32](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L32) | [L38](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S3PredicateOnlyTest.kt#L38) | Both sides open the file first, and the right click is the same. The difference is what comes with it: Driver's `openFile()` also waits until the file is analysed, so one call is enough. The Remote Robot `openFile()` in `pages/IdeaFrame.kt` only posts the request, so the test waits for the editor itself.                                                                                                                       |
| 2. Take the popup | [L35](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L35) | [L42](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S3PredicateOnlyTest.kt#L42) | Both sides take the popup as an object and search inside it, `popupMenu()` on one and `JPopupMenuFixture` on the other. The popup is its own window either way, so neither lookup starts from the IDE frame.                                                                                                                                                                    |
| 3. Describe the entries as two collections | [L38](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L38) | [L46](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S3PredicateOnlyTest.kt#L46) | Driver ships `PopupItemUiComponent`, which reads a label with `getText()`. The library has nothing for a single entry, so `pages/ActionMenuFixture.kt` writes the wrapper, the xpath and the label getter, and that getter is one more `callJs()`. `JPopupMenuFixture` does offer `collectItems()`, but it cannot tell a leaf from a submenu, and `jMenu()` can, but only from an exact label.                                                                                 |
| 4. Poll the submenus with a predicate | [L44](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L44) | [L50](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S3PredicateOnlyTest.kt#L50) | `waitForOne()` returns only when exactly one element matches, and it hands that element back. This is what makes the predicate safe here, because two entries contain the word Paste. The Remote Robot side builds the same thing by hand: `count { } == 1` inside the wait, then `single { }` after it. The wait gives back only a boolean, so the search runs a second time and can see a different menu than the wait did.           |
| 5. Click it | [L55](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L55) | [L59](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S3PredicateOnlyTest.kt#L59) | Identical.                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 6. The submenu really opened | [L60](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L60) | [L63](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S3PredicateOnlyTest.kt#L63) | Both sides check that there are more menu entries on screen than before. On the Remote Robot side reading that number throws while the menu is still redrawing, so the wait has to be `waitForIgnoringError()`. Driver's `xx().list()` returns an empty list instead of throwing, so a plain `waitFor()` is enough. Its error-tolerant wait, `shouldBe()`, is not an option here anyway: it only works on a component, not on a number. |
| 7. Close the submenu | [L65](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L65) | [L68](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S3PredicateOnlyTest.kt#L68) | Identical.                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 4. How the wrapper is found | [L22](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/pages/ProjectPanelUi.kt#L22) | [L37](remote-robot/src/test/kotlin/com/example/migration/remoterobot/pages/ProjectPanelFixture.kt#L37) | A function returning `x(MyUi::class.java) { }` (starter) with a typed query, versus a `@DefaultXpath` annotation (Remote Robot) holding a raw xpath string that nothing checks until the test runs. The Starter receiver is `IdeaFrameUI`, so the call is offered only where the panel can be found.                                                                                                                             |
| 5. Use the wrapper | [L20](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S5PageObjectTest.kt#L20) | [L27](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S5PageObjectTest.kt#L27) | Same shape on both sides.                                                                                                                                                                                                                                                                              |
| 6. Assert through it | [L23](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S5PageObjectTest.kt#L23) | [L30](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S5PageObjectTest.kt#L30) | Driver has `shouldBe()` and `hasSubtext()`. Remote Robot collects the rows and searches them by hand inside a `waitForIgnoringError()`.                                                                                                                                                                      |

One thing the tables cannot show is how much is already written. `remote-fixtures` ships about twenty fixtures and they are all plain Swing widgets: a button, a label, a list, a table, a tree, a text editor, a popup menu. `driver-sdk` ships over a hundred component classes, and most of them are IDE parts rather than widgets: the frame, the main toolbar, the navigation bar, a dozen tool windows, the Settings dialog, the completion lookup, the welcome screen, plus panels that belong to single products. So a hand-written page object is the exception on the Starter side, while on the Remote Robot side almost everything above the widget level ends up in `pages/`, which is what happened here.
